...
# === 反向豁免阈值（V3 基于业界研究新增）===
# fork 比例健康 → 跳过所有 marketing 判定。
# 依据：Awesome Agents 实测 Flask/LangChain/AutoGPT 等健康项目 stars/forks ≈ 6~10；
# dev.to 实战文章给出"forks 占 stars 10-30% 即健康"区间（即 stars/forks ≈ 3.3~10）。
# 取 15 作为保守的健康上限——比业界基线宽松，留出余量避免误伤。
# 理由：刷星每个 0.5 元，刷 fork 要每个号都建仓库，业界共识 fork 比例是最难造假、
# 也是最强的"真有人用"反向证据。
_FORK_HEALTHY_RATIO = 15

# 路径 B（冷启动）仅在体量较小时启用——已有体量的项目日波动几千很正常。
# 依据：Awesome Agents "嫌疑线"明确要求总 stars > 10000 才适用绝对值判据；
# 反向取此值作为冷启动上限——stars >= 此值 → 跳过路径 B（因为它本来就只该用于冷启动）。
_COLD_START_MAX_STARS = 10000
...
def judge(item: dict, history: dict | None = None) -> str:
    ...
    # V3 反向豁免：fork 比例健康即跳过所有 marketing 判定（业界首要健康信号）
    is_fork_healthy = stars / max(forks, 1) <= _FORK_HEALTHY_RATIO

    if not is_fork_healthy:
        ...
        if snap is not None and snap.get("age_days", 0) >= _MIN_HISTORY_AGE_DAYS:
            ...
            if (
                ...
                return "marketing"
        elif stars < _COLD_START_MAX_STARS:
            # 路径 B · 冷启动：用 stars_today 近似（仅对小体量项目适用）
            if (
