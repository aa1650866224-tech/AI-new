...
**硬性长度约束（违反视为输出不合格，会被自动截断）：**
- chinese_summary：**严格 130-170 中文字符**（不是英文单词数）。超过 170 字请删字精简，不要展开。少于 130 字请补充关键数字。
- editor_note：**严格 30-80 中文字符**。超过 80 字请精简到一句话。

请在输出前自己数一下字数，确认在范围内再返回。
...
{{
  ...
  "chinese_summary": "130-170 字的客观摘要，2-3 句话。新闻通稿口吻，可以分点列举关键信息（'核心功能包括：A；B；C'）。包含核心事件 + 关键数字/产品名/版本号。禁止'值得关注''重磅'等评价性套话。如果原文是英文，直接消化产出中文，不要先翻译再压缩。优先保留：发布方、核心数字、版本号；删掉：背景铺垫、官方溢美词。",
