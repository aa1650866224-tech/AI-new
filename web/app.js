    ...
    filtered.forEach((item, idx) => {
        ...
        card.innerHTML = `
            <div class="card-header">
                <div class="card-title" data-id="${itemId}">${idx + 1}. ${escapeHtml(item.chinese_title || item.title)} ${domainTag}</div>
                <div class="card-badges">${srcBadge} ${catBadge} ${impBadge}</div>
            </div>
            <div class="card-summary" data-id="${itemId}">${escapeHtml(item.chinese_summary || item.summary || item.content || '')}</div>
            <div class="card-meta">
                <span>${heat} &nbsp;|&nbsp; ${escapeHtml(item.author || 'unknown')} &nbsp;|&nbsp; ${item.sentiment || 'neutral'}</span>
